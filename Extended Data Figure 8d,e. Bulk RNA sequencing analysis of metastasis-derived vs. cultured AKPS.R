...
# Subset data to include specific columns
metalines <- all_samples_mat[, c( 2, 10, 25, 22, 23)]
head(metalines)
...
corrected_DEGs <- na.omit(topTags(qlf.2vs1, n = Inf, adjust.method = "fdr", sort.by = "PValue", p.value = 1)$table)
View(corrected_DEGs)
...
DEGs_volcano(corrected_DEGs, 0.02, 3, "met-derived vs. culture", "grey",9, 10) +ggsave("metderived_volcano.pdf", width =8,  height=8)
...
BP_enrichment <- preranked_BP(et2v1$table)
View(BP_enrichment)

#plot
ggplot(BP_enrichment %>% filter(abs(NES)>2.3 & padj<0.01) %>% head(n= 100), aes(reorder(pathway, NES), NES)) +
  ...
  theme_classic()+
  labs(y="")+   
  theme(axis.text.y=element_text(size=10)) + ggsave("metderivedvsculture_BP.pdf", width =15,  height=8)
